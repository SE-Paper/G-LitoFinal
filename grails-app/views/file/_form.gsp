<%@ page import="groovylito.user.File" %>
...
<div class="fieldcontain ${hasErrors(bean: fileInstance, field: 'post', 'error')} required">
	...
	<g:select id="post" name="post.id" from="${groovylito.user.Post.list()}" optionKey="id" required="" value="${fileInstance?.post?.id}" class="many-to-one"/>
