...
<div class="fieldcontain ${hasErrors(bean: postInstance, field: 'forum', 'error')} required">
	...
	<g:select id="forum" name="forum.id" from="${groovylito.user.Forum.list()}" optionKey="id" required="" value="${postInstance?.forum?.id}" class="many-to-one"/>
...
<div class="fieldcontain ${hasErrors(bean: postInstance, field: 'regular', 'error')} required">
	...
	<g:select id="regular" name="regular.id" from="${groovylito.user.Regular.list()}" optionKey="id" required="" value="${postInstance?.regular?.id}" class="many-to-one"/>
